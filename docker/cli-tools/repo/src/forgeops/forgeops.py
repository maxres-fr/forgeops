...
def create_release_notes(args):
    ...
    body = {'tag_name': args.tag_name,
            ...
            'body': notes,
            'draft': True}
    res = requests.post('https://api.github.com/repos/maxres-fr/forgeops/releases',
...
def main():
    ...
    # prior tag
    prior_tag_parser = subparsers.add_parser('prior-tag', help='determine the tag prior to the current tag')
    prior_tag_parser.add_argument('-t', '--tag-name', help='use this tag as the current tag instead of the current commit')
    prior_tag_parser.set_defaults(func=find_last_tag)

    # current tag
    current_tag_parser = subparsers.add_parser('current-tag', help='print the current tag')
    current_tag_parser.set_defaults(func=current_tag)

    # create release
    release_parser = subparsers.add_parser('create-release', help='create a draft release on github with notes')
    release_parser.add_argument('-t', '--tag-name', help='tag name of release. defaults to active tag', default=active_tag())
    release_parser.add_argument('notes', type=argparse.FileType('r'), default=sys.stdin)
